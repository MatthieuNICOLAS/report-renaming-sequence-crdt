...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \Function{recomputeMapIds}{$S: State, seq: <Int, Int>[]$}{$: Map<Id, Id>$}
      \State $mapIds \gets Map()$
      \State $n \gets seq.length$
      \ForAll {$(id_{site}, clock_{site}, index) \in seq$}
        \ForAll {$(id, elt) \in S$}
          \If{$id.id_{site} = id_{site}$ AND $id.clock_{site} = clock_{site}$}
            \State $id' \gets renameIdentifier(id, index, n)$
            \State $mapIds.set(id, id')$
          \EndIf
        \EndFor
...
\begin{itemize}
  ...
  \item Is not an issue if we are not able to find an element of the sequence
    in the current state
  \begin{itemize}
    \item Happens if the element has been deleted concurrently
    \item Thus we won't need to rename it
  \end{itemize}
