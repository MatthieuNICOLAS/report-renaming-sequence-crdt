...
\begin{itemize}
  ...
  \end{itemize}
  \item Performances of these operations depend on the number of elements
  ...
  \item Should be able to adapt the algorithms \ref{alg:local-rename} and \ref{alg:remote-rename}
    to Identifier-based Sequence \acp{CRDT} with variable granularity
  \begin{itemize}
    \item Here, the granularity of element is 1 ($add$ and $delete$ one by one)
